...
def show():
    ...
    try:
        ...
        try:
            offset = (page - 1) * EVENTS_LIMIT
            res = requests.get(f"{API_URL}/events?page={page}&per_page={EVENTS_LIMIT}", headers=headers, timeout=DEFAULT_TIMEOUT)
            res.raise_for_status()
            if res.status_code == 200:
                ...
                if isinstance(events, dict) and 'events' in events:
                    ...
                    with col1:
                        if page > 1:
                            if st.button("◀️ Previous", key="previous_button"):
                                st.session_state.event_page -= 1
                                st.rerun()
                    with col2:
